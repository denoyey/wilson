                            ...
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex justify-end gap-2">
                                        <button wire:click="openEditModal({{ $category->id }})" class="text-blue-600 hover:text-blue-900 transition-colors bg-blue-50 hover:bg-blue-100 p-1.5 rounded-md" title="Edit">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                                        </button>
                                        <button wire:click="confirmDelete({{ $category->id }})" class="text-red-600 hover:text-red-900 transition-colors bg-red-50 hover:bg-red-100 p-1.5 rounded-md" title="Hapus">
                        ...
                        <form wire:submit="save" class="space-y-4">
                            ...
                            <div class="flex justify-end gap-3 pt-4 border-t border-gray-200">
                                ...
                                <button type="submit" class="px-3 py-1.5 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700 transition-colors shadow-sm" wire:loading.attr="disabled">
                                    <span wire:loading.remove wire:target="save">{{ $isEditing ? 'Simpan Perubahan' : 'Tambah Kategori' }}</span>
                ...
                <div class="flex items-center justify-center min-h-screen px-4">
                    ...
                    <div class="relative bg-white rounded-lg shadow-xl w-full max-w-md p-6 z-10">
                        <div class="flex items-center gap-4 mb-4">
                            <div class="flex-shrink-0 h-12 w-12 rounded-full flex items-center justify-center {{ $cannotDelete ? 'bg-orange-100' : 'bg-red-100' }}">
                                <svg class="h-6 w-6 {{ $cannotDelete ? 'text-orange-600' : 'text-red-600' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    @if($cannotDelete)
                            ...
                            <div>
                                <h3 class="text-lg font-bold text-gray-900">
                                    {{ $cannotDelete ? 'Tidak Bisa Dihapus' : 'Hapus Kategori' }}
                                </h3>
                                <p class="text-sm text-gray-500 mt-1">
                                    @if($cannotDelete)
                                        Kategori <strong>"{{ $deletingCategoryName }}"</strong> tidak bisa dihapus karena masih digunakan oleh beberapa barang. Anda harus memindahkan atau menghapus barang tersebut terlebih dahulu.
                                    @else
                                        Apakah Anda yakin ingin menghapus kategori <strong>"{{ $deletingCategoryName }}"</strong>? Tindakan ini tidak bisa dibatalkan.
                                    @endif
                                </p>
                            </div>
                        </div>
